...
{	
	if(nIDEvent == 1)
	{
		m_strCurrentTime = m_FPlayer.GetCurrentTimeDisplay();
		m_strPlayTime.Format("%s / %s", m_strCurrentTime, m_FPlayer.GetTotalTimeDisplay());
		...
		if( nLength > 0 )
		{
			...
			m_strCurrentTime.Format("%2d:%s", nCurTime/60, szCur2);
			m_strPlayTime.Format("%2d:%s / %2d:%s", nCurTime/60, szCur2, nLength/60, szLen2);
			m_nCurPosition = nCurTime*100/nLength;	
...
{	
	m_FPlayer.Open(m_strCurrentMp3Path);//m_Player.Open(m_strCurrentMp3Path);
